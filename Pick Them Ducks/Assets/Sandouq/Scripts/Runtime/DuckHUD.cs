...
namespace Sandouq.Ducks
{
 public sealed class DuckHUD:MonoBehaviour
 {
  ...
  [SerializeField] Text[] titles=new Text[5],values=new Text[5],costs=new Text[5],slotText=new Text[5];[SerializeField] Button[] buys=new Button[5],slots=new Button[5],tabs=new Button[3];int page;bool reset;float next; bool fifthRowEnabled; Text[] buyLabels=new Text[5]; DuckUIAnimation[] slotAnimations=new DuckUIAnimation[5],tabAnimations=new DuckUIAnimation[3];
  ...
  void Bind()
  {
   ...
   for(int i=0;i<5;i++){int index=i;slotAnimations[i]=slots[i].GetComponent<DuckUIAnimation>();slotColors[i]=slots[i].GetComponent<Image>().color;slotTextColors[i]=slotText[i].color;slots[i].onClick.RemoveAllListeners();slots[i].onClick.AddListener(()=>game.Equip(index));buys[i].onClick.RemoveAllListeners();buys[i].onClick.AddListener(()=>Buy(index));}
   for(int i=0;i<3;i++){int index=i;tabAnimations[i]=tabs[i].GetComponent<DuckUIAnimation>();tabColors[i]=tabs[i].GetComponent<Image>().color;tabs[i].onClick.RemoveAllListeners();tabs[i].onClick.AddListener(()=>{page=index;Refresh();});}
  ...
  public void Refresh()
  {
   ...
   for(int i=0;i<5;i++){slotAnimations[i]?.SetSelected(d.currentTool==i);slots[i].GetComponent<Image>().color=d.currentTool==i?selectionColor:slotColors[i];slotText[i].color=d.currentTool==i?selectionTextColor:slotTextColors[i];slotText[i].text=toolNames[i]+(d.owned[i]?"":"\nLOCKED");slots[i].interactable=d.owned[i];}
   ...
   for(int i=0;i<3;i++){tabs[i].GetComponent<Image>().color=page==i?selectionColor:tabColors[i];tabAnimations[i]?.SetSelected(page==i);}if(!reset)subtitle.text=d.money.ToString("N0")+" coins / "+p.Equipment.name+" / "+d.casketKits+" casket kits";
