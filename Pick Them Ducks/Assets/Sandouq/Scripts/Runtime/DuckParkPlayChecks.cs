...
namespace Sandouq.Ducks
{
    public sealed class DuckParkPlayChecks : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            int fillId=29000,fillIndex=0;while(game.Physics.RegularBodyCount<96&&fillId<35000){if(game.Population.IsAvailable(fillId)){var point=game.Park.Land(new Vector3(75+(fillIndex%12),0,210+fillIndex/12));if(game.Physics.Launch(fillId,point+Vector3.up*10,Vector3.zero))fillIndex++;}fillId++;}
            try{Check(game.Physics.RegularBodyCount==96,"Saturate rigidbody pool");Check(game.CollectId(pileBase)&&game.Physics.FallingCount>0,"Pile starts collapsing with no free Rigidbody");}catch(Exception e){Fail(e);yield break;}
            yield return new WaitForSeconds(.45f);
            try{Check(game.Population.Position(pileBase+1).y<crowdedPile.y+.5f,"Physical pile actually falls despite regular pool saturation");Check(game.feedbackComponent.rollerCollectionAudio!=null&&game.particles!=null,"Authored sound and particle hook components");}catch(Exception e){Fail(e);yield break;}
